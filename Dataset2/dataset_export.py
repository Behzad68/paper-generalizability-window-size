...
- The time stamp of some sensors are not the same, so I use the time stamp of the accelerator sensor.
...
- During the writing code, the priority was mostly being readable than efficiency, so feel free to improve it  
'''

activities = None


# labels = pd.read_csv('/home/behzad68/PycharmProjects/extract_recofit_dats/activities_labels.csv')
...
def _extract_info(struct):
    ...
    activity_label = labels.loc[labels['activity'] == struct.activityName, 'label_all'].values[0]
    records = np.insert(records, records.shape[1], activity_label, axis=1)
    return records
...
def export_to_csv(matfile_path, path_to_save):
    ...
    pd.DataFrame(activities).to_csv(os.path.join(path_to_save, 'activities.csv'), index=False)  # export avtivity names
    ...
    df.to_csv(os.path.join(path_to_save, 'exercise_data.50.0000_singleonly_all_labels.csv'), index=False)
    print("done")
...
# start here
mat_file = os.path.join(os.getcwd(), 'raw_dataset', 'exercise_data.50.0000_singleonly.mat')
path_to_save = os.path.join(os.getcwd(), 'raw_dataset')
export_to_csv(matfile_path=mat_file, path_to_save=path_to_save)
